...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  int p=0;
  __cs149_vec_float x,result,upperBound;
  upperBound = _cs149_vset_float(9.999999f);
  __cs149_vec_int y,one,zero;
  __cs149_mask maskAll, maskAct;
  one = _cs149_vset_int(1);
  zero = _cs149_vset_int(0);
  maskAll = _cs149_init_ones();
  for(;p<N-VECTOR_WIDTH;p+=VECTOR_WIDTH)
  {
    _cs149_vload_float(x, values+p, maskAll);
    _cs149_vload_int(y, exponents+p, maskAll);
    _cs149_vset_float(result, 1.0, maskAll);
    _cs149_vgt_int(maskAct, y, zero, maskAll);
    // printf("maskAct=%d\n", _cs149_cntbits(maskAct));
    for(int _=0;_<EXP_MAX;_++)
    {
      _cs149_vmult_float(result, result, x, maskAct);
      _cs149_vsub_int(y, y, one, maskAct);
      _cs149_vgt_int(maskAct, y, zero, maskAll);
      // printf("maskAct=%d\n", _cs149_cntbits(maskAct));
      __cs149_mask maskOverflow;
      _cs149_vgt_float(maskOverflow, result, upperBound, maskAll);
      _cs149_vset_float(result, 9.999999f, maskOverflow);
      __cs149_mask maskZero = _cs149_mask_not(maskAct);
      if(_cs149_cntbits(maskZero) == VECTOR_WIDTH)
        break;
    }
    _cs149_vstore_float(output+p, result, maskAll);
  }
  int len=N-p;
  if(len > 0) {
    maskAll = _cs149_init_ones(len);
    _cs149_vload_float(x, values+p, maskAll);
    _cs149_vload_int(y, exponents+p, maskAll);
    _cs149_vset_float(result, 1.0, maskAll);
    _cs149_vgt_int(maskAct, y, zero, maskAll);
