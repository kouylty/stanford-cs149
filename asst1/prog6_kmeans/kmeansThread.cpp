...
void fthread(int lm, int rm, WorkerArgs *const args, double *minDist) {
...
void computeAssignments(WorkerArgs *const args) {
  ...
  */
  ...
  for(int i=0;i<numThreads;i++)
  {
    ...
    //   }
    // }
    t.push_back(thread(fthread, i*args->M/numThreads, (i+1)*args->M/numThreads, args, minDist));
  }
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    printf("[Iteration %d] computeAssignments took %.3f ms\n", iter,(endTime - startTime) * 1000);
    startTime = CycleTimer::currentSeconds();
    computeCentroids(&args);
    endTime = CycleTimer::currentSeconds();
    printf("[Iteration %d] computeCentroids took %.3f ms\n", iter,(endTime - startTime) * 1000);
    startTime = CycleTimer::currentSeconds();
    computeCost(&args);
    endTime = CycleTimer::currentSeconds();
    printf("[Iteration %d] computeCost took %.3f ms\n", iter,(endTime - startTime) * 1000);
